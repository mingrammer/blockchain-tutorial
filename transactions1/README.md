...
트랜잭션은 비트코인의 핵심이며 블록체인의 유일한 목적은 트랜잭션을 안전하고 신뢰할 수 있는 방식으로 저장하는 것이기 때문에 트랜잭션이 생성되면 그 누구도 이를 수정할 수 없다. 이번 파트에서는 트랜잭션을 구현할 것이다. 그러나 이는 꽤 큰 주제이기 때문에 두 파트로 나눌 것이다. 이 파트에서는 일반적인 트랜잭션 메커니즘을 구현할 것이며 두 번째 파트에서는 좀 더 자세한 내용을 다룰 것이다.
