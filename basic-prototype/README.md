...
이제 블록체인을 구현해보자. 블록체인은 본질적으로 데이터베이스이며, 이는 순서가 있고 뒤로 연결되는 리스트라는 특별한 구조를 지닌다. 이는 블록들이 삽입 순서대로 저장되며 각 블록은 이전 블록에 연결됨을 의미한다. 이러한 구조 덕분에 최신 블록을 빠르게 가져올 수 있고 해시로 블록을 (효율적으로) 검색할 수 있다.
